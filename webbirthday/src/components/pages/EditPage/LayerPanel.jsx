...
export default function LayerPanel(){
    ...
    return (
        <div id ="layer-container" className="w-[15%] h-screen bg-[#f8fafd] shadow-md shadow-black">
            <div id="layer-navigate" className="w-full h-[4rem]">
                <div className="w-[80%] h-[50%] ml-4">
                    Trở về, ẩn tab ở đây
                </div>
            ...
            <div id="id_list_container" className="w-full h-[80%]">
                <LayerContext.Provider value={{layerSelected,setlayerSelected}}>
                    <div id="id_list" className="ml-[1.5rem]">
                        {temp_layers.map((layer,index)=>{
                            return (
                                <Layer key={index} id={`layer_${index}`} layerGivenName={layer}></Layer>
                            );
                        })}
                    </div>
...
export function Layer({layerGivenName="Unnamed layer", id}){
    ...
    return (
        <div>

            {
                (layerSelected!=id)?
                <div 
                onClick={()=>{setlayerSelected(id)}}
                className="flex w-[90%] h-[1.5rem] items-center rounded-[4px] px-[0.5rem] hover:bg-[#cde1f7]">
                    <input type="text" value={layerName} onChange={(e)=>{handleChangeLayerName(e)}} className="border-0 text-[1rem] bg-transparent focus:border-0 focus:outline-0"></input>
                </div>
                :
                <div className="flex w-[90%] h-[1.5rem] items-center rounded-[4px] px-[0.5rem] bg-[#cde1f7]">
                    <input type="text" value={layerName} onChange={(e)=>{handleChangeLayerName(e)}} className="border-0 text-[1rem] bg-transparent focus:border-0 focus:outline-0"></input>
                </div>
            }
